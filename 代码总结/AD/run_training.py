...
version 6: for SGD, head_layer=8, cutpaste_normal,
"""
...
def run_training(data_type="screw",
    ...
    writer = SummaryWriter(Path("logdirs_test2") / model_name)
    ...
    if optim_name == "sgd":
        ...
        print(f"ERROR unknown optimizer: {optim_name}")
        return

    # load the model
    if load_model:
        # find the model
        if len(list(Path(args.model_dir).glob(f"model-{data_type}*"))) == 0:
            print(f"there are no model for {data_type}")
        load_model_name = list(Path(args.model_dir).glob(f"model-{data_type}*"))[0]
    ...
    for step in tqdm(range(epochs)):
        ...
        if test_epochs > 0 and epoch % test_epochs == 0:
            ...
            # TODO: we might not want to use the training datat because of droupout etc.
            #  but it should give a indecation of the model performance???
            # batch_embeds = torch.cat(batch_embeds)
            # print(batch_embeds.shape)
            model.eval()
            roc_auc = eval_model(data_dir, model_name, data_type,
                                 device=device,
                                 save_plots=False,
                                 size=size,
    ...
    torch.save({'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, model_dir / f"{model_name}.tch")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', default="all",
                        help='MVTec defection dataset type to train separated by , '
    ...
    parser.add_argument('--data_dir', default='data', help='folder of input img, (default: data)')
    ...
    parser.add_argument('--no-pretrained', dest='pretrained', default=True, action='store_false',
                        help='use pretrained values to initialize ResNet18 , (default: True)')

    parser.add_argument('--test_epochs', default=10, type=int,
                        help='interval to calculate the auc during training, if -1 do not calculate test scores, '
                             '(default: 10)')
    ...
    parser.add_argument('--optim', default="sgd",
                        help='optimizing algorithm values:[sgd, adam] (default: "sgd")')

    parser.add_argument('--load_model', default=True,
                        help='if load model from tch file: [True, False] (default: True)')

    parser.add_argument('--batch_size', default=64, type=int,
                        help='batch size, real batch_size is depending on cut paste config normal cutaout has effective'
                             'batch_size of 2x batch_size (default: "64")')
    ...
    parser.add_argument('--variant', default="3way", choices=['normal', 'scar', '3way', 'union'],
                        help='cutpaste variant to use (default: "3way")')
