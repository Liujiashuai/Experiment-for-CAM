"""
when model is training, test the AUC in image-level
"""
...
def get_train_embeds(data_dir, model, size, defect_type, transform, device):
...
def eval_model(data_dir, modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True,
               model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    # create test dataset
    ...
    if test_data_eval is None or cached_type != defect_type:
        ...
        test_data_eval = MVTecAT(data_dir, defect_type, size, transform=test_transform,
                                 gt_transform=gt_transform, mode="test")
    ...
    if model is None:
        ...
        if torch.cuda.is_available():
            model_check = torch.load(modelname)
        else:
            model_check = torch.load(modelname, map_location=torch.device('cpu'))
    ...
    embeds = []
    with torch.no_grad():
        for x, _, label in dataloader_test:
            embed, logit = model(x.to(device))
    ...
    if train_embed is None:
        train_embed = get_train_embeds(data_dir, model, size, defect_type, test_transform, device)
    ...
    if save_plots:
        ...
        if show_training_data:
            ...
            train_data = MVTecAT(data_dir, defect_type, transform=train_transform, size=size, mode='train')
            dataloader_train = DataLoader(train_data, batch_size=32, shuffle=True, num_workers=8,
                                          collate_fn=cut_paste_collate_fn, persistent_workers=True)
            ...
            # for tsne we encode training Data as 2, and augmented Data as 3
...
def plot_roc(labels, scores, filename, modelname=None, save_plots=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', default="all",
                        help='MVTec defection dataset type to train seperated by , '
                             '(default: "all": train all defect types)')
    ...
    parser.add_argument('--density', default="torch", choices=["torch", "sklearn"],
                        help='density implementation to use. '
                             'See `density.py` for both implementations. (default: torch)')
    ...
    # find models
    model_names = [list(Path(args.model_dir).glob(f"model-{data_type}*"))[0]
                   for data_type in types if len(list(Path(args.model_dir).glob(f"model-{data_type}*"))) > 0]
    ...
    for model_name, data_type in zip(model_names, types):
        ...
        roc_auc = eval_model(data_dir=args.data_dir, modelname=model_name, defect_type=data_type,
                             save_plots=args.save_plots, device=device, head_layer=args.head_layer, density=density())
