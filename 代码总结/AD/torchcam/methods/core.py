        ...
        **kwargs: Any,
        ...
        with torch.no_grad():
            for weight, activation in zip(weights, self.hook_a):
                ...
                # print("the weight1 is", weight)  # for test

                weight = weight[(...,) + (None,) * missing_dims]

                # print("the weight2 is", weight)  # for test
                # print("the activation is", activation)  # for test
                # print("the shape to test", weight.shape, activation.shape)  # for test

                # Perform the weighted combination to get the CAM
                cam = torch.nansum(weight * activation, dim=1)  # type: ignore[union-attr]

                # print("the cam is", cam)  # for test
