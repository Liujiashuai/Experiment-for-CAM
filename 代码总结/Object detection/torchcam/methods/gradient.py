...
__all__ = ['GradCAM', 'GradCAMpp', 'SmoothGradCAMpp', 'XGradCAM', 'LayerCAM', 'SmoothLayerCAM']
...
class LayerCAM(_GradCAM):
    ...
    def _scale_cams(cams: List[Tensor], gamma: float = 2.) -> List[Tensor]:
        ...
        return [torch.pow(cam, 2) for cam in cams]  # new scale function


class SmoothLayerCAM(_GradCAM):
    def __init__(
        self,
        model: nn.Module,
        target_layer: Optional[Union[Union[nn.Module, str], List[Union[nn.Module, str]]]] = None,
    ...
    def _get_weights(
        ...
        **kwargs: Any
        ...
        for _idx in range(self.num_samples):
            ...
            self._backprop(out, class_idx, **kwargs)
            # sum the grad
            grad_1 = [g1.add_(grad) for g1, grad in zip(grad_1, self.hook_g)]

        # Average the gradient
        grad_1 = [g1.div_(self.num_samples) for g1 in grad_1]

        return [torch.relu(grad) for grad in grad_1]
